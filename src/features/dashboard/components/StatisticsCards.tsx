  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card className={cn(
        ...
        attendance && attendance.days_present > (attendance.total_required_workdays / 2)
          ? "border-emerald-100"
          : "border-slate-100"
      )}>
        <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
        ...
        <CardContent>
          ...
          {attendance && attendance.days_present > (attendance.total_required_workdays / 2) && (
            <div className="flex items-center gap-2 mt-4 animate-in fade-in slide-in-from-top-1 duration-500">
              <Badge
                variant="outline"
      ...
      <Card className={cn(
        "shadow-sm hover:shadow-md transition-all border",
        attendance && ((attendance.late_minutes || 0) > 60 || (attendance.early_leave_minutes || 0) > 60)
          ? "border-amber-100"
          : "border-slate-100"
      )}>
        <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Typography
                ...
                className={cn(
                  ...
                  (attendance?.late_minutes || 0) > 60 ? "text-amber-600" : "text-slate-700"
                )}
              >
            ...
            <div>
              <Typography
                ...
                className={cn(
                  "text-xl font-bold block",
                  (attendance?.early_leave_minutes || 0) > 60 ? "text-amber-600" : "text-slate-700"
                )}
              >
